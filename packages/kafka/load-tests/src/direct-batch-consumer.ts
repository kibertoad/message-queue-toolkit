...
export class DirectBatchConsumer extends AbstractKafkaConsumer<typeof DIRECT_TOPICS_CONFIG, ExecutionContext, true> {
  constructor(metrics: MetricsCollector, batchSize = 50, timeoutMs = 200) {
    ...
    super(
      deps,
      {
        ...
        handlers: new KafkaHandlerRoutingBuilder<typeof DIRECT_TOPICS_CONFIG, ExecutionContext, true>()
          .addConfig(
            'direct-events',
            new KafkaHandlerConfig(DIRECT_EVENT_SCHEMA, (messages, ctx) => {
              console.log(`[${new Date().toISOString()}] [batch-handler] direct-events batch received: ${messages.length} messages`)
          ...
          .addConfig(
            ...
            new KafkaHandlerConfig(DIRECT_ORDER_SCHEMA, (messages, ctx) => {
              console.log(`[${new Date().toISOString()}] [batch-handler] direct-orders batch received: ${messages.length} messages`)
              for (const message of messages) {
