...
describe('PermissionPublisher', () => {
  ...
  describe('publish', () => {
    ...
    it('should fail if there is no schema for message type', async () => {
      ...
      // When
      await expect(publisher.publish('permission-added', message)).rejects.toThrow(InternalError)
    })
    ...
    it('should publish only messages meeting schema', async () => {
      ...
      // When&Then - invalid
      await expect(publisher.publish(PERMISSION_ADDED_TOPIC, messageInvalid)).rejects.toThrow(
        InternalError,
      )
    })
  })
